...
当你试图在单张 A100（80GB）上训练一个 7B 参数的模型时，你很快会发现：**光是模型参数 + 优化器状态就需要约 112GB 显存**，一张卡根本放不下。
...
传统数据并行（DDP）在每次反向传播后需要 **All-Reduce 全部梯度**，通信量为 $2\Phi$ bytes × 2（发送 + 接收）。当模型达到数十亿参数时，通信开销可能超过计算时间。
