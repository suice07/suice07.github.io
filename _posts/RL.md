...
确定，将状态空间的一个点映射到动作空间的一个点，这也就是决策过程。
...
函数参数固定，只调整策略函数 ；
    ...
    函数另外单独训练，训练方式同 Q-learning

作者理解：

寻找最优决策：DPG的最终目的还是在于调整策略，所以先假设已有最优的critic:
，在 时刻遇到某个状态 ，要求此刻最优动作 ，我们需要做的就是固定住 函数输入端 当中的状态 为 ，不断调整输入动作 ，直到 的输出值最大，此时的动作 即为最优决策

，这个操作等价于一般的Q-learning中的greedy决策方式：
